# -*- coding: utf-8 -*-
"""
Research Agent MCP Server — expose all capabilities as MCP tools.

Run: python -m research_agent.integrations.mcp_server
...
def create_server():
    if not HAS_MCP:
        print("MCP not installed. Install: pip install research-agent[mcp]", file=sys.stderr)
        sys.exit(1)

    server = Server("research-agent")
    config = Config()
    eyes = ResearchAgent(config)
    ...
    async def list_tools():
        return [
            ...
            Tool(name="get_status",
                 description="Get Research Agent status: which channels are active.",
